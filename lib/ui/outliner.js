...
export class Outliner extends Pane {
    constructor(scene, camera, camControl, renderer, physics, assetManager) {
        super({
    ...
    #createVisualPane() {
        const visualFolder = this.addFolder({
            title: '🎨 Visual', expanded: false,
        });

        visualFolder.addBinding(this.assetManager, 'xray', {
            label: 'X-Ray',
        }).on('change', (event) => {
            this.assetManager.setXray(event.value);
        })
    }
}
